    ...
    for i in range(len(cls_segms)):
        for cls_segm in cls_segms[i]:
            encoded_mask_results[i].append(
                mask_util.encode(np.array(cls_segm[:, :, np.newaxis],
                                          order='F',
...
def mask_target(pos_proposals_list, pos_assigned_gt_inds_list, gt_masks_list, cfg):
    ...
    mask_targets = map(mask_target_single,
                       pos_proposals_list,
...
def mask_target_single(pos_proposals, pos_assigned_gt_inds, gt_masks, cfg):
    ...
    if num_pos > 0:
        ...
        mask_targets = gt_masks.crop_and_resize(proposals_np,
                                                mask_size,
                                                device=device,
...
class BitmapMasks:
    ...
    def rescale(self, scale, interpolation='nearest'):
        ...
        if len(self.masks) == 0:
            ...
            rescaled_masks = np.stack([mmcv.imrescale(mask, scale, interpolation=interpolation)
    ...
    def resize(self, out_shape, interpolation='nearest'):
        ...
        if len(self.masks) == 0:
            ...
            resized_masks = np.stack([mmcv.imresize(mask, out_shape[::-1], interpolation=interpolation)
                                      for mask in self.masks])
    ...
    def flip(self, flip_direction='horizontal'):
        ...
        if len(self.masks) == 0:
            ...
            flipped_masks = np.stack([mmcv.imflip(mask, direction=flip_direction)
                                      for mask in self.masks])
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        if len(self.masks) == 0:
            ...
            padded_masks = np.stack([mmcv.impad(mask, shape=out_shape, pad_val=pad_val)
                                     for mask in self.masks])
    ...
    def translate(self,
        ...
        if len(self.masks) == 0:
            ...
            translated_masks = mmcv.imtranslate(self.masks.transpose((1, 2, 0)),
                                                offset,
                                                direction,
...
def polygon_to_bitmap(polygons, height, width):
    ...
    bitmap_mask = mask_util.decode(rle).astype(np.bool)
    return bitmap_mask
